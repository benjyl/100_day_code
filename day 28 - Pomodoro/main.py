...
def count_down(count):
    ...
    if count > 0:
        ...
        tick_labels.config(
            text=int((reps) / 2) * "✔"
        )  # add tick for every work session completed
...
)  # highlightthickness sets thickness of border around canvas
filename = PhotoImage(
    file="tomato.png"
)  # reads through a file and gets hold of particular image at particular file loc
canvas.create_image(
    100, 112, image=filename
)  # image cut off when using x = 100, so if move to right slightly (103), see whole tomato
