...
while len(correct_guesses) < 50:
    ...
    answer_state = screen.textinput(
        title=f"{len(correct_guesses)}/50 States guessed correctly",
    ...
    if (
        answer_state in data["state"].unique() and answer_state not in correct_guesses
    ):  # could also turn data["state"] into list
